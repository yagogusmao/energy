...
export default class GerenciadorController extends Component {
        ...
        this.setState({ carregando: true }, () =>
            ...
            Api.listarVeiculosSemEquipe()
                ...
                if (res[3].data.veiculo === {}) {
                    this.setState({ _id, tipo, status, veiculo, local, apontamentos, funcionarios, funcionariosSemEquipe, carregando: false, veiculos: res[4].data.veiculos })
                } else {
                    const { modelo, numeracao, kilometragem } = res[3].data.veiculo;
                    this.setState({ _id, modelo, numeracao, kilometragem, tipo, status, veiculo, local, apontamentos, funcionarios, funcionariosSemEquipe, carregando: false, veiculos: res[4].data.veiculos })
                }
            })
        )
    }
    ...
    definirVeiculoEquipe = (_idVeiculo) => {
        ...
        Api.definirVeiculo({ _id, veiculo: _idVeiculo }).then(res => {
            Api.verVeiculo(_id).then(res1 => {
                this.setState({ veiculos: res.data.veiculos, veiculo: res1.data.veiculo._id, modelo: res1.data.veiculo.modelo, numeracao: res1.data.veiculo.numeracao, kilometragem: res1.data.veiculo.kilometragem },
                    () => this.growl.show({ severity: 'success', summary: res.data.mensagem }))
            })
        }, erro => this.growl.show({ severity: 'error', summary: erro.response.data.mensagem })
        )
    ...
    retirarVeiculo = () => {
        ...
        Api.deletarVeiculo(_id).then(res => {
            this.setState({ veiculos: res.data.veiculos, veiculo: "", modelo: "", numeracao: "", kilometragem: "" },
                () => this.growl.show({ severity: 'success', summary: res.data.mensagem }))
        }, erro => this.growl.show({ severity: 'error', summary: erro.response.data.mensagem })
        )
    }

    actionTemplateButton = (rowData) => <Button style={{ backgroundColor: '#ce5f52' }} label="Retirar da equipe" onClick={() => { this.retirarFuncionario(rowData._id) }} className="p-button-raised p-button-rounded" />

    actionTemplateButtonAdicionar = (rowData) => <Button style={{ backgroundColor: '#ce5f52' }} label="Adicionar à equipe" onClick={() => { this.adicionarFuncionario(rowData._id) }} className="p-button-raised p-button-rounded" />

    actionTemplateButtonVeiculo = (rowData) => <Button style={{ backgroundColor: '#ce5f52' }} label="Definir veículo da equipe" onClick={() => { this.definirVeiculoEquipe(rowData._id) }} className="p-button-raised p-button-rounded" />
